...
bot.callbackQuery(/approve_(\d+)_(\d+)_(\d+)_(\d+)/, async (ctx) => {
    ...
    if (parseInt(countRes.rows[0].count) === 100) {
        ...
        for(let p of players.rows) {
            await bot.api.sendMessage(p.owner_id, "🔔 ሁሉም ቲኬቶች ተሽጠዋል! የቀጥታ ዕጣ ማውጣት በ 5 ደቂቃ ውስጥ ይጀምራል። አሁኑኑ አፑን ከፍተው ይጠብቁ!");
        }
        setTimeout(() => runDrawLogic(tId, rnd), 300000); // 5 Mins
    }
    await bot.api.sendMessage(uId, `✅ ክፍያዎ ተቀብለናል! ቁጥር #${num} አሁን የእርስዎ ነው።`);
